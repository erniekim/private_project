...
def getTopRank(today, logger, conn):
    ...
    while page_size > 1:
        ...
        daum_url = 'http://land.naver.com/article/articleList.nhn?rletTypeCd=A01&tradeTypeCd=A1&rletNo=105159&hscpTypeCd=A01&page='+ str(page)
        ...
        for index, keword in enumerate(resultList) :
            ...
            if i > 9 :
                ...
                cursor.execute(sql_query, (today, Date, BuildName, BuildID, BuildNum, Floor, Price, Description, City, State, Region,))
                conn.commit()
        logger.info('')
...
def Create_DB_Tabel(conn):
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE realestateprice(id varchar(20) primary key,date varchar(20), buildname varchar(100), buildnum varchar(20), floor varchar(20), price varchar(20), description varchar(200), city varchar(20), state varchar(20), region varchar(20));")
    conn.commit()
    cursor.close()
...
def Daemon():
    ...
    navigator = bs4.BeautifulSoup(html_content, "html.parser")
    resultList = navigator.find_all('a')
